...
class Block:
    ...
    def rectangles_to_draw(self) -> List[Tuple[Tuple[int, int, int],
        ...
        """
        # === Precondition ===
        # - This block has been initialized by random_init, so that children is
        #   not None(it's a list of either length 0 or 4).
        # - The method update_block_locations() has been called beforehand,
        #   so that all the blocks within this block have its position and size.

        result = []
        if len(self.children) == 0:
            # Draw every rectangles.
            result.append(
                (self.colour,
                 self.position,
                 (self.size, self.size),
            ...
            # And draw the frame of every rectangles.
            result.append(
                (FRAME_COLOUR,
                 self.position,
                 (self.size, self.size),
            ...
            for child in self.children:
                result += child.rectangles_to_draw()
    ...
    def update_block_locations(self, top_left: Tuple[int, int],
        ...
        if len(self.children) == 4:
            x, y = top_left[0], top_left[1]  # (x, y) coordinate of this block
            size_child = round(size / 2)  # size of each child
            self.children[0].update_block_locations(
                (x + size_child, y), size_child
            )
            self.children[1].update_block_locations(
                (x, y), size_child
            )
            self.children[2].update_block_locations(
                (x, y + size_child), size_child
            )
            self.children[3].update_block_locations(
                (x + size_child, y + size_child), size_child
            )
    ...
    def get_selected_block(self, location: Tuple[int, int], level: int) \
        ...
        if self.level < level:
            if len(self.children) == 4:
                # Position of self.children[3] is the center position of
                # this block if this block is subdivided.
                location_diff = (
                    location[0] - self.children[3].position[0],
                    location[1] - self.children[3].position[1]
                )
                if location_diff[0] >= 0 > location_diff[1]:  # 1 quadrant
                    return self.children[0].get_selected_block(location, level)
                elif location_diff[0] < 0 and location_diff[1] < 0:
                    # 2 quadrant
                    return self.children[1].get_selected_block(location, level)
                elif location_diff[0] < 0 <= location_diff[1]:  # 3 quadrant
    ...
    def flatten(self) -> List[List[Tuple[int, int, int]]]:
        ...
        where, for 0 <= i, j < 2^{max_level - self.level}
        ...
        result = []
        if len(self.children) == 0:
            lst_size = 2 ** (self.max_depth - self.level)
            for _ in range(lst_size):
                result.append([self.colour] * lst_size)
        else:  # len(self.children) == 4
            lst_size1 = 2 ** (self.max_depth - self.children[0].level)
            for i1 in range(lst_size1):
                result.append(
                    self.children[1].flatten()[i1] +
                    self.children[2].flatten()[i1]
                )
            for i2 in range(lst_size1):
                result.append(
                    self.children[0].flatten()[i2] +
                    self.children[3].flatten()[i2]
                )
...
def random_init(level: int, max_depth: int) -> 'Block':
    ...
    If the level of a block is not at the maximum depth, the probability of
    this block being subdivided is exp(-0.25 * <level>).
    ...
    if level < max_depth and random.random() < math.exp(-0.25 * level):
        child_list = []
        for _ in range(4):
            child_list.append(random_init(level + 1, max_depth))
        random_block = Block(level, children=child_list)
        random_block.max_depth = max_depth
    else:
        random_block = Block(level, colour=COLOUR_LIST[random.randint(0, 3)])
        random_block.max_depth = max_depth
    return random_block
